...
namespace udl_crc32_details
{
	void make_crc_table( u32 _crcTable[] )
	{
		...
		do
		{
			...
			_crcTable[ (size_t)b ] = remainder;
		} while ( 0 != ++b );
	};

	static constexpr u32 crc32_table[] = {
	...
	constexpr u32 calc_crc32( const char *_string, const size_t _size, const u32 _crc )
	{
		return ( _size == 0 ) ? ~_crc
							  : calc_crc32( &_string[ 1 ], _size - 1, crc32_table[ ( ( _crc ) ^ ( _string[ 0 ] ) ) & 0xff ] ^ ( ( _crc ) >> 8 ) );
	}
}

static constexpr u32 operator"" _crc32( const char *_str, const size_t _size )
{
	return udl_crc32_details::calc_crc32( _str, _size, 0xffffffff );
}